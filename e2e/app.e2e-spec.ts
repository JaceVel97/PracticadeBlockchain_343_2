...
describe('Starting tests for angular-app-6', function() {
  ...
  it('website title should be angular-app-6', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('angular-app-6');
    })
  });

  it('network-name should be tienda4@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('tienda4@0.0.1.bna');
    });
  });

  it('navbar-brand should be angular-app-6',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('angular-app-6');
    });
  });
    ...
    it('Producto table should have 10 columns',() => {
      page.navigateTo('/Producto');
    ...
    it('Rebastecer component should be loadable',() => {
      page.navigateTo('/Rebastecer');
      browser.findElement(by.id('transactionName'))
      .then((transactionName) => {
        return transactionName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('Rebastecer');
      });
    });
